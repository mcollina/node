...
function emitHookFactory(symbol, name) {
  ...
  const fn = function(asyncId, resource) {
    active_hooks.call_depth += 1;
    ...
    try {
      for (var i = 0; i < active_hooks.array.length; i++) {
        if (typeof active_hooks.array[i][symbol] === 'function') {
          active_hooks.array[i][symbol](asyncId, resource);
        }
      }
    } catch (e) {
      fatalError(e);
...
function emitBeforeScript(asyncId, triggerAsyncId, r) {
  ...
  if (async_hook_fields[kBefore] > 0)
    emitBeforeNative(asyncId, r);
}


function emitAfterScript(asyncId, resource) {
  validateAsyncId(asyncId, 'asyncId');

  if (async_hook_fields[kAfter] > 0)
    emitAfterNative(asyncId, resource);
...
module.exports = {
  ...
  registerDestroyHook,
  currentResource: async_wrap.currentResource
};
